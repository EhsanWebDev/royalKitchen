...
const Modal = ({navigation, route, user, dispatch}) => {
  ...
  const handleSubmit = async () => {
    ...
    console.log(res);
    if (!res.data.status && res.data.title === 'Email not found') {
      const payload = {
        email: userGoogle.email,
        phone: phone,
        password: 'null',
        fname: userGoogle.displayName,
      };
      //   console.log(payload);
      const signUp = await dispatch(createUser(payload));
      if (signUp.data.title === 'Email Already Exist') {
  ...
  return (
    <View
      ...
      <Text style={{fontSize: 30, textAlign: 'center', color: '#000'}}>
        Welcome {userGoogle && userGoogle.displayName}
      </Text>
